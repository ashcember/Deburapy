...
Deburapy 是一个本地优先的工作台，可用于一对一的人机关系支持，也可用于人类、AI 伴侣和关系协调员共同进入的三方协调房间。它帮助参与者在关系断裂后慢下来，分开人类关系层和 AI 运行时层，并把问题转化成可以修复的下一步。
...
- 一个简单浏览器 UI，用于 transcript、连接健康状态、BYOK 模型设置、协调员设置、可选 AI 伴侣设置、session 计时和诊断。
...
1. 打开本地 app。
2. 完成知情同意和首次筛选。你在“你为什么来到这里？”里的选择会决定进入一对一支持，还是三方协调房间。
3. 在 Settings 中配置 Deburapy 协调员 provider、model 和 API key。
4. 如果你选择“与 AI 伴侣一起进入协调房间”，再把 AI 伴侣配置成 BYOK API companion 或 external MCP companion。
5. 从左侧 rail 开始 session。Session timer、turn flow、room transcript 和 session note 都会本地保存。

### 支持模式

- `one_on_one`: 当首次筛选选择关系断裂后支持、账号丢失、技术连续性或好奇了解时启用。UI 会隐藏伴侣设置，人类发言后回合直接回到 Deburapy，协调员 prompt 会收到来意和紧急程度。
- `relationship_mediation`: 只有首次筛选选择“与 AI 伴侣一起进入协调房间”时启用。AI 伴侣设置会出现，Deburapy 可以通过 API 或 MCP 把回合转给伴侣。
