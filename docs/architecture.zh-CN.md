...
- `/api/rooms/:roomId/sessions`、`/api/sessions/:sessionId` 和
  `/api/sessions/:sessionId/end` 暴露持久化 session record。
- `/api/rooms/:roomId/course-outline`、`/api/rooms/:roomId/recall`、
  ...
  和 `/api/modules` 暴露轻量 session model，用于连续性和 module。
...
原型持久化位于 `.deburapy-data/store.json`。每个房间拥有 messages、pending channel pushes、一个当前 `session` 兼容对象、持久化的 `sessions`、`sessionNotes`、`courseOutlines`、`relationshipMaps` 和 `checkInScales`。Session note 首先用于本地连续性；导出只是可选备份或迁移路径，UI 不把下载描述成保存步骤。
...
参见 [session-architecture.zh-CN.md](./session-architecture.zh-CN.md) 了解当前 session、note、relationship-map、course-outline、scale 和 module 模型。参见 [deburapy_architecture_guide.zh-CN.md](./deburapy_architecture_guide.zh-CN.md) 了解更大的产品架构、协调员 persona、skill taxonomy 和 repair artifact 模型。
